...
# data_frame <- data_frame[which(data_frame$queryMetadata.query == c("influenza")), ]
data_frame <- data_frame[which(data_frame$queryMetadata.query == c("flushot")), ]
...
left <- plot_usmap(data = df_left, values = "Frequency", lines = "black")+
  ...
  labs(title = "Fequency of flu related tweets for keywords \n flushot from each state")+
...
for(i in c(1:2)){
  data_frame <- data_frame_all[which(data_frame_all$queryMetadata.query == 'flushot'), ]
