...
public class WordCount {
    ...
    public static class TokenizerMapper
        ...
        private String processedText(String rawText){
            //rawText = removeUrl(rawText);
            rawText = rawText.replaceAll("[^a-zA-Z ]", "").toLowerCase();
            return rawText;
        }

        private String removeUrl(String rawText) {
            String urlPattern = "((https?|ftp|gopher|telnet|file|Unsure|http):((//)|(\\\\))+[\\w\\d:#@%/;$()~_?\\+-=\\\\\\.&]*/)";
            Pattern pattern = Pattern.compile(urlPattern,Pattern.CASE_INSENSITIVE);
            Matcher matcher = pattern.matcher(rawText);
            int i = 0;
            while (matcher.find()) {
                rawText = rawText.replaceAll(matcher.group(i),"").trim();
                i++;
            }
            return rawText;
        }
        ...
        public void map(Object key, Text value, Context context
            ...
            String processedText = processedText(rawText);
            StringTokenizer itr = new StringTokenizer(processedText);
            while (itr.hasMoreTokens()) {
                String token = itr.nextToken();
                if(!stopwords.contains(token) && token.length() >=5){
                    word.set(token);
